...
-- Fix public.normalize_greek() Unicode escape sequences.
--
-- In 001_init.sql the mapping strings were accidentally written with double backslashes
-- (e.g. U&'\\1F00'), which causes Postgres to treat them as literal backslash characters
-- instead of Unicode escape sequences. The result is effectively `lower(input_text)` with
-- diacritics preserved, which breaks diacritics-insensitive prefix search.
...
-- Backfill stored normalized columns so existing data starts matching immediately.
