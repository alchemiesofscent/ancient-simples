...
- `UI-SIMPLES-CANDIDATES-01` `[UI]` Determine and implement a candidate-simple classification/view that separates likely named simples from generic parts, lemmatization errors, and audit-only extraction terms. Status: `not_started`
...
- `UI-SIMPLES-WORK-FILTER-01` `[UI]` Made work-level filtering explicit: internal Source filters/codes → user-facing Work filters with readable labels; Registry-to-Evidence handoff without work context → handoff preserving selected work; cross-corpus detail panes under a work filter → work-scoped hits, forms, samples, and evidence examples. Status: `implemented`
