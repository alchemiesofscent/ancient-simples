...
- Source: CMG repo's existing cross-author lemma linking work
- Start with Dioscorides ↔ Galen SMT (~400+ matching substance chapters)
- Output: `config/alignments/seed.jsonl`
- Depends on CMG repo (G-01) for structural analysis data

AL-03: `scripts/import_alignments.py`
- Reads alignment JSONL → creates `entry_alignments` rows
- Maps display_entry_ids to tei_entries.id via lookup

AL-04: Schema for entry_alignments (included in S-01)

AL-05: Alignment review UI (deferrable to after Phase 3 core UI)

Track E: Dioscorides CSV supplements

DIOSC-01: Import translations from `diosc.csv`
- Match CSV rows to TEI entry_ids via bridge mapping (book.chapter → segment_id)
- Create `translations` version 1 rows with provenance: source_file='diosc.csv', import_method='csv_bridge'

DIOSC-02: Classify var_par_prod_gr annotations
- Pass 1: automated lookup of top ~80 known terms against parts_vocab + process_vocab
- Pass 2: remaining terms queued for v3 pipeline or manual review
- Output: assertions with source='diosc_vpp_import', status='draft'

DIOSC-03: Seed Dioscorides lemma candidates
- Run `match_diosc_lemmata.py` against CSV headword column
- Create lemma_forms with source='csv_bridge', status='draft'
- Generate audit CSV flagging category mismatches for human review

DIOSC-04: Lemma matching audit
- 821 unique Dioscorides lemmata; ~411 match existing by normalized form
- Audit CSV: exact matches (93), orthographic variants (318), genuinely new (410)
- All matching requires human review before confirmation
